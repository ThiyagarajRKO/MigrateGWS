...
export class MigrationExecutionCoordinator extends EventEmitter {
  ...
  private createOptimizedPhases(
    ...
    // Phase 1: Fast & Low-Risk Services (Contacts, Calendar, Groups)
    const phase1Services = services.filter(s => ['contacts', 'calendar', 'groups'].includes(s));
    ...
    // Phase 2: Collaboration Services (Chat, Forms, Slides)
    const phase2Services = services.filter(s => ['chat', 'forms', 'slides'].includes(s));
    ...
    if (phase4Services.length > 0) {
      phases.push({
        ...
        prerequisites: ['drive'], // Media might reference Drive items
      });
    }
